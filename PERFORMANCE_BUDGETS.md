...
|---|---:|---:|---|
| `stage1.driver` | 480 | 524,288 | active compiler construction from the frozen seed |
| `seed_bootstrap` | 480 | 1,048,576 | reused-stage1 fixed-point comparison |
| `proof` | 1,200 | 3,145,728 | copied-selfhost, corpus, and tooling proof |
...
The values include deliberate CI variance above the measured M53
ownership-aware baseline. The composed gate builds stage1 once, then reuses its
seed-emitted C and executable for the fixed-point comparison; it does not pay a
second seed emission. Change ceilings only in `scripts/performance_budgets.sh`,
backed by a controlled baseline and review. Do not relax a ceiling merely to
hide an unexplained regression.
